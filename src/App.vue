...
import useAddToHomeScreen from "./composables/use-add-to-homescreen";

const emojis = ["💦", "💧", "🌊", "💦", "💧", "🌊", "🐬"];
const { deferredPrompt, captureEvent, clickCallback } = useAddToHomeScreen();
...
const appContext = useStorage("appContext", { installed: false });
...
<template>
  ...
  <footer v-if="deferredPrompt || true">
    <Button @click="clickCallback">Add to home screen</Button>
  </footer>
</template>
